...
export function Home() {
  ...
  const handleLimparPortfolio = async () => {
    const ok = await confirm({
      ...
      message:
        "Deseja limpar todos os dados importados pela planilha principal?\n\nSer�o removidos: Cursos, Plano de Metas, Valores PCA, Cursos por Eixo, Visitas, Horas, A��es Extensivas e Eventos. Usu�rios, CEPED, hist�rico e logs permanecem.",
      destructive: true,
    ...
    try {
      ...
      setResultados(resultado.resultados);
      toastSuccess("Dados importados dos 8 mÃ³dulos foram removidos.");
      refreshUi();
    } catch (error) {
      console.error(error);
